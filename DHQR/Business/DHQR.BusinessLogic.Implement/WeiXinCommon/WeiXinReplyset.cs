...
namespace DHQR.BusinessLogic.Implement
{
    public class WeiXinReplyset
    {
        ...
        WeiXinMenuLogic caidandal = new WeiXinMenuLogic();//自定义菜单服务

        WeiXinFirstInLogic firstService = new WeiXinFirstInLogic();//首次关注服务
        WeiXinKeyWordLogic keyWordService = new WeiXinKeyWordLogic();//关键字服务
        WeiXinTriggerInfoLogic triggerInfoService = new WeiXinTriggerInfoLogic();//图文触发服务
        WeiXinPicMsgMatserLogic picMsgService = new WeiXinPicMsgMatserLogic();//图文消息服务
        WeiXinPicMsgDetailLogic picDetailService = new WeiXinPicMsgDetailLogic();//图文明细服务

        WeiXinCommonApi wxCommand = new WeiXinCommonApi();

        JavaScriptSerializer Jss = new JavaScriptSerializer();

        public WeiXinReplyset()
        { }
        ...
        public string GetDefault(string FromUserName, string ToUserName, Guid weiXinAppId)
        {
            ...
            return resXml;
             */
            var resXml = "<xml><ToUserName><![CDATA[" + FromUserName + "]]></ToUserName><FromUserName><![CDATA[" + ToUserName + "]]></FromUserName><CreateTime>" + ConvertDateTimeInt(DateTime.Now) + "</CreateTime><MsgType><![CDATA[text]]></MsgType><Content><![CDATA[" + "无匹配信息" + "]]></Content><FuncFlag>0</FuncFlag></xml>";
            return resXml;
        }
        #endregion 默认回复
        ...
        public string GetKeyword(string FromUserName, string ToUserName, string Content,Guid weiXinAppId,string weiXinKey)
        {
            ...
            WeiXinKeyWord dtKeyword=keyWordService.GetByContent(weiXinAppId,Content);
        ...
        public string GetMenuClick(string FromUserName, string ToUserName, string EventKey,Guid WeixinAppId,string WeixinKey)
        {
            ...
            try
            {
                 WeiXinMenu dtMenu=caidandal.GetByEventKey(WeixinAppId,EventKey);
                if (dtMenu!=null)
                {
        ...
        {
            string resXml = "";
            WeiXinTriggerInfo triggerInfo = triggerInfoService.GetByKey(masterId);
            if (triggerInfo != null)
            {
