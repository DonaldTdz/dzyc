...
var web_obj = {
    web_config_init: function (action) {
        frame_obj.file_upload($('#MusicUpload'), '', '', '', false, 1, function (filepath) {
            $('#config_form input[name=MusicPath]').val(filepath);
        }, '*.mp3', 500);
        frame_obj.file_upload($('#PagesPicUpload'), $('#config_form input[name=PagesPic]'), $('#PagesPicDetail'), action);
        $('#PagesPicDetail').html(frame_obj.upload_img_detail($('#config_form input[name=PagesPic]').val()));
    },
    ...
    home_init: function () {
        //加载上传按钮
        frame_obj.file_upload($('#HomeFileUpload'), $('#home_form input[name=ImgPath]'), $('#home .web_skin_index_list').eq($('#home_form input[name=no]')).find('.img'));
        for (var i = 0; i < 5; i++) {
            frame_obj.file_upload($('#HomeFileUpload_' + i), $('#home_form input[name=ImgPathList\\[\\]]').eq(i), $('#home_form .b_r').eq(i));
        }
    ...
    home_code_init: function () {
        ...
        $('#home_code_form').submit(function () {
            $('#home_code_form input:submit').attr('disabled', true);
            return true;
        });
    },
    ...
    column_edit_init: function () {
        if ($('#ColumnFileUpload').size()) {
            frame_obj.file_upload($('#ColumnFileUpload'), $('#column_form input[name=ImgPath]'), $('#ColumnImgDetail'), 'web_column');
            $('#ColumnImgDetail').html(frame_obj.upload_img_detail($('#column_form input[name=ImgPath]').val()));
        }

        for (var i = 0; i < 5; i++) {
            frame_obj.file_upload($('#BannerUpload_' + i), $('#column_form input[name=Banner\\[\\]]').eq(i), $('#column_form .r_bar').eq(i));
            if ($('#column_form input[name=Banner\\[\\]]').eq(i).val()) {
                $('#column_form .r_bar').eq(i).html('<img src="' + $('#column_form input[name=Banner\\[\\]]').eq(i).val() + '" />');
            }
        }

        $('#column_form a[href=#web_home_img_del]').click(function () {
            var i = $('#column_form a[href=#web_home_img_del]').index($(this));
            $('#column_form input[name=Banner\\[\\]]').eq(i).val('');
            $('#column_form .r_bar').eq(i).html('');
        });

        $('#column_form select[name=UrlList\\[\\]]').each(function () {
            $(this).find("option[value='" + $('input[name=Url_' + $('#column_form select[name=UrlList\\[\\]]').index($(this)) + ']').val() + "']").attr('selected', true);
        });

        var ext_link_fun = function () {
            if ($('#column_form input[name=ExtLink]:checked').size()) {
                $('#Description_rows, #ListType_rows, #Banner_rows').hide();
                $('#LinkUrl_span').show();
            } else {
                $('#Description_rows, #ListType_rows, #Banner_rows').show();
                $('#LinkUrl_span').hide();
                form_init_fun();
            }
        }
    ...
    column_article_init: function () {
        if ($('#ColumnAtricleFileUpload').size()) {
            frame_obj.file_upload($('#ColumnAtricleFileUpload'), $('#column_article_form input[name=ImgPath]'), $('#ColumnAtricleImgDetail'), 'web_column_article');
            $('#ColumnAtricleImgDetail').html(frame_obj.upload_img_detail($('#column_article_form input[name=ImgPath]').val()));
        }
        if ($('#ColumnAtricleFileUploadIndex').size()) {
            frame_obj.file_upload($('#ColumnAtricleFileUploadIndex'), $('#column_article_form input[name=ImgPathIndex]'), $('#ColumnAtricleImgDetailIndex'), 'web_column_article');
            $('#ColumnAtricleImgDetailIndex').html(frame_obj.upload_img_detail($('#column_article_form input[name=ImgPathIndex]').val()));
        }

        $('#column_article_form select[name=CId]').change(function () {
            $('#file_upload_size').html(columnType[$(this).val()]);
        });
    ...
    products_category_init: function () {
        if ($('#CategoryFileUpload').size()) {
            frame_obj.file_upload($('#CategoryFileUpload'), $('#web_category_form input[name=PicPath]'), $('#CategoryImgDetail'), 'web_category');
            $('#CategoryImgDetail').html(frame_obj.upload_img_detail($('#web_category_form input[name=PicPath]').val()));
        }
    ...
    lbs_init: function () {
        frame_obj.file_upload($('#LbsFileUpload'), $('#lbs_form input[name=ShopImg]'), $('#ShopImgDetail'));
        $('#ShopImgDetail').html(frame_obj.upload_img_detail($('#lbs_form input[name=ShopImg]').val()));
        frame_obj.map_init();
    ...
    //---------- diy页面 ----------//
    changeCtrl: function () {
        $(".sp1 .title1").each(function (index, element) {
            $(element).click(function () {
                $(this).addClass("whitebg").siblings().removeClass("whitebg");
                $(".sp2").eq(index).show().siblings(".sp2").hide();
            });
        });
    },
    delChange: function () {
        $(".sp2_del").click(function () {
            if (confirm("您确定要删除该背景吗？")) {
                $(".sp2_content").html("");
                $("#hid_bg").val("");
                $(".ipad").css("background-image", "");
            }
        })
    },
    diy_init: function () {
        web_obj.changeCtrl(); //控件面板与背景转换
        web_obj.delChange(); //删除背景
        ...
        //------ 组件：一行三列p0上传 -------//
        frame_obj.file_upload($("#upfile_p0_0"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p0ImgFrame").eq(0).find(".imgObj").html('<img src="' + path + '" width="95" />');
            }
        });
        frame_obj.file_upload($("#upfile_p0_1"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p0ImgFrame").eq(1).find(".imgObj").html('<img src="' + path + '" width="95" />');
            }
        });
        frame_obj.file_upload($("#upfile_p0_2"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p0ImgFrame").eq(2).find(".imgObj").html('<img src="' + path + '" width="95" />');
            }
        });
        //------ 组件：一行两列p1上传 -------//
        frame_obj.file_upload($("#upfile_p1_0"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .imgObj").eq(0).html('<img src="' + path + '" width="146" />');
            }
        });
        frame_obj.file_upload($("#upfile_p1_1"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .imgObj").eq(1).html('<img src="' + path + '" width="146" />');
            }
        });
        //------ 组件：图片p3上传 -------//
        frame_obj.file_upload($("#upfile"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .imgObj").html('<img src="' + path + '" width="292" />');
            }
        });
        //------ 组件：图片p4上传 -------//
        frame_obj.file_upload($("#upfile_p4_0"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p4ImgFrame img").eq(0).attr("src", path).show().siblings().hide();
                $("#p4LookDetail0").html('<img src="' + path + '" width="50" />');
            }
        });
        frame_obj.file_upload($("#upfile_p4_1"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p4ImgFrame img").eq(1).attr("src", path);
                $("#p4LookDetail1").html('<img src="' + path + '" width="50" />');
            }
        });
        frame_obj.file_upload($("#upfile_p4_2"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p4ImgFrame img").eq(2).attr("src", path);
                $("#p4LookDetail2").html('<img src="' + path + '" width="50" />');
            }
        });
        frame_obj.file_upload($("#upfile_p4_3"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p4ImgFrame img").eq(3).attr("src", path);
                $("#p4LookDetail3").html('<img src="' + path + '" width="50" />');
            }
        });
        frame_obj.file_upload($("#upfile_p4_4"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p4ImgFrame img").eq(4).attr("src", path);
                $("#p4LookDetail4").html('<img src="' + path + '" width="50" />');
            }
        });
        //------ 组件6：一行四列p6上传 -------//
        frame_obj.file_upload($("#upfile_p6_0"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p6ImgFrame").eq(0).find(".imgObj").html('<img src="' + path + '" width="95" />');
            }
        });
        frame_obj.file_upload($("#upfile_p6_1"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p6ImgFrame").eq(1).find(".imgObj").html('<img src="' + path + '" width="95" />');
            }
        });
        frame_obj.file_upload($("#upfile_p6_2"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p6ImgFrame").eq(2).find(".imgObj").html('<img src="' + path + '" width="95" />');
            }
        });
        frame_obj.file_upload($("#upfile_p6_3"), '', '', 1, true, 1, function (path) {
            if ($(".selectObj").hasClass(pName)) {
                $(".selectObj .dragPart .p6ImgFrame").eq(3).find(".imgObj").html('<img src="' + path + '" width="95" />');
            }
        });
        //------ 版面背景 -------//
        frame_obj.file_upload($("#upfile_bg"), '', '', 1, true, 1, function (path) {
            $(".sp2_content").html("<img src='" + path + "' />");
            $("#hid_bg").val(path);
            $(".ipad").css({ "background": "url(" + path + ")", "background-size": "cover" });
        });
    },
    //组件元素操作
    "Evt1": "",
    "Evt2": "onmouseover='this.style.border=\"1px #3270AD solid\"' onmouseout = 'this.style.border = \"1px #F0F0EE solid\"' ",
    "Evt3": "onmouseover='this.src=\"" + system_obj.domain('static') + "/member/images/web/diy/select_1.png\"' onmouseout = 'this.src=\"" + system_obj.domain('static') + "/member/images/web/diy/select.png\"'",
    "cloIco": system_obj.domain('static') + "/member/images/web/diy/del.png",
    "firstrproperty": "",
    ...
    "p6": { //一行四列
        "packageElement": function (packageName) {
            ...
            $str += "</div>";
            $str += "</div>";
            $str += "<div class='delObj hand' onclick='web_obj.delObjEvt(this,\"p6\");'><img " + web_obj.Evt1 + " src='" + web_obj.cloIco + "' /></div>";
            $str += "<div class='clean'></div>";
            $str += "</div>";
            return $str;
        },
    ...
    "btnCtrl": function () {
        ...
        $.ajax({
            type: "POST",
            url: "?",
            data: "pagebg=" + $("#hid_bg").val() + "&gruopPackage=" + encodeURIComponent(str) + "&do_action=web.home_diy&date=" + new Date(),
            dataType: "json",
