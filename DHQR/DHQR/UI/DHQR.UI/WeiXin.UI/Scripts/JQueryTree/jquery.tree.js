...
//部署路径Scripts/JQueryTree/
var path = t.substring(0, t.lastIndexOf("/") - 18);
(function ($) {
    ...
    $.fn.treeview = function (settings) {
        var dfop =
            {
                ...
                url: false, //异步请求的url
                cbiconpath: path+"Content/JQueryTree/Images/Icons/", //checkbox icon的目录位置
                icons: ["checkbox_0.gif", "checkbox_1.gif", "checkbox_2.gif"],
                emptyiconpath: path+"Content/JQueryTree/Images/s.gif", //checkbxo三态的图片
                showcheck: false, //是否显示checkbox
                showFavorite: false,
                oncheckboxclick: false, //当checkstate状态变化时所触发的事件，但是不会触发因级联选择而引起的变化
                onnodeclick: false, // 触发节点单击事件
                onplusclick: false, //触发节点展开事件
                onfavclick: false, //触发收藏事件
                cascadecheck: true, //是否启用级联，默认启用
                data: null, //初始化数据
                clicktoggle: true, //点击节点展开和收缩子节点
                theme: "bbit-tree-lines" //三种风格备选：bbit-tree-lines,bbit-tree-no-lines,bbit-tree-arrows
            };

        $.extend(dfop, settings); //用传进来的参数覆盖默认，没传则保留
        var treenodes = dfop.data;
        var showFavorite = dfop.showFavorite;
        var me = $(this);
        var id = me.attr("id");
        if (id == null || id == "") {
            id = "bbtree" + (+new Date);
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            ht.push("<div id='", id, "_", nid, "' value='", nd.value, "' tpath='", path, "' unselectable='on' title='", nd.text, "'");
            var cs = [];
            ...
            ht.push("</a>");
            //======================================
            //如果数据中包含收藏菜单字段，则添加一个收藏图标
            //======================================
            if (!nd.hasChildren) {
                if (showFavorite) {
                    if (!nd.IsFavoriteItem)
                        ht.push("<span class='bbit-tree-favorite ui-icon ui-icon-star' pid='", nid, "'></span>");
                    else
                        ht.push("<span class='bbit-tree-favorite ui-icon ui-icon-trash' pid='", nid, "'></span>");
                } 
            }
            ht.push("</div>");
            //Child
        ...
        function nodeclick(e) {
            ...
            if (et.tagName == "IMG") {
                ...
                if (dfop.onplusclick) {
                    dfop.onplusclick.call(this, item);
                }
            }
            else {
                //==如果点击了收藏图标，则弹出收藏菜单========================================================
                if ($(et).hasClass("bbit-tree-favorite") && showFavorite) {
                    if (dfop.onfavclick) {
                        dfop.onfavclick.call(this, item);
                        if ($(et).hasClass("ui-icon-star")) {
                            $(et).removeClass("ui-icon-star");
                            $(et).addClass("ui-icon-trash");
                        }
                        else {
                            $(et).removeClass("ui-icon-trash");
                            $(et).addClass("ui-icon-star");
                        }
                    }
                    dfop.citem = item;
                    return;
        ...
        function expendTo(itemId) {
            if(itemId == undefined || itemId == null || itemId == ""){
                return;
            }
            var data = dfop.data;
            var parentNodes = [];
            getParents(data, parentNodes, itemId);
            if (parentNodes.length == 0)
                parentNodes.push(itemId);
            else
                parentNodes.reverse();
            for (var i = 0; i < parentNodes.length; i++) {
                var parentId = parentNodes[i];
                var targetId = "#" + id + "_" + parentId.replace(/[^\w]/gi, "_");
                var imgId = targetId + " img.bbit-tree-ec-icon";
                var target = $(targetId);
                var img = $(imgId);
                target.click();
                img.click();
        ...
        function getParents(nodes, results, value) {
            if (nodes == null || nodes.length == 0 || value == null)
                return;
            for (var i = 0; i < nodes.length; i++) {
                ...
                if (item.id == value) {
                    ...
                    if (item.ParentId != null) {
                        getParents(dfop.data, results, item.ParentId);
                    }
                    break;
                }
                else if (item.ChildNodes != null) {
                    getParents(item.ChildNodes, results, value);
                }
            }
        }
        function getItemById(data,id){
            for (var i = 0; i < data.length; i++) {
               ...
               if(item.ChildNodes != null){
                 var subItem = getItemById(item.ChildNodes,id);
                 if(subItem != null){
                    return subItem;
                 }
               }
        ...
        me[0].t = {
            ...
            getItemById:function(itemId){
                ...
                return getItemById(dfop.data,itemId);
            },
    ...
    //展开到指定id的节点
    $.fn.expendTo = function (id) {
        if (this[0].t) {
            this[0].t.expendTo(id);
        }
        return null;
    };
    //重新绘制树
    ...
    $.fn.getItemById = function(id){
        if(this[0].t){
           return this[0].t.getItemById(id);
        }
        return null;
    };
})(jQuery);