...
function fPopCalendar(evt,popCtrl,dateCtrl){
    ...
    with ($$("calendardiv").style) {
        //张园修改
        if (point.x + $$("calendardiv").offsetWidth > document.body.offsetWidth) {
            left = point.x + popCtrl.offsetWidth - $$("calendardiv").offsetWidth;
        } else {
            left = point.x + "px";
        }
        //
...
function fSetDate(iYear,iMonth,iDay){
    ...
    //增加失去焦点
    if($("#txtIDCard").val()!=undefined)
    {
     $("#txtBirthDay").blur();
    } 
    if(gdCtrl.name=="txtExtendedDeliveryTime")
    {
      TimeExtendedDelivery(gdCtrl,gdCtrl.value);
    }
    fHideCalendar();
   
}
function fHideCalendar(){
$$("calendardiv").style.visibility="hidden";for(var i=0;i<goSelectTag.length;i++){goSelectTag[i].style.visibility="visible";}goSelectTag.length=0;
}
...
function clearDate(){gdCtrl.value = '';fHideCalendar();  if($("#txtIDCard").val()!=undefined){ $("#txtBirthDay").blur()}}//增加失去焦点
...
function getDateDiv(){
    ...
    var dateDiv="";
    dateDiv += "<div id='calendardiv'  onclick='event.cancelBubble=true' " + noSelectForIE + " style='" + noSelectForFireFox + "position:absolute;z-index:999999999;visibility:hidden;border:1px solid #999999;width:174px;'>";
    dateDiv += "<table style='z-index:10000;' border='0' bgcolor='#E0E0E0' cellpadding='1' cellspacing='1' width='100%'  >";
    dateDiv+="<tr>";
