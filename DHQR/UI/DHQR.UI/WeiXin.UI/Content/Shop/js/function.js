...
//删除选中值
function DeleteSelectedDatas(objId, objmsg, callback) {
    if ($(".checkall:input:checked").size() < 1) {
        $.ligerDialog.warn("对不起，请选中您要操作的记录！");
        return false;
    }
    ...
    $.ligerDialog.confirm(msg, "提示信息", function (result) {
        if (result) {
            var retunObj = new Array();
            $(".checkall:input:checked").each(function () {
                var val = $(this).val();
                retunObj.push(val);
            })
            if (typeof (callback) == "function") callback(retunObj);
        }
    });
    return false;
}
...
function Upload(action, repath, uppath, iswater, isthumbnail, filepath) {
    var sendUrl = "../../FileUp/UpLoadFile?action=" + action + "&ReFilePath=" + repath + "&UpFilePath=" + uppath;
    //判断是否打水印
    if (arguments.length == 4) {
        sendUrl = "../../FileUp/UpLoadFile?action=" + action + "&ReFilePath=" + repath + "&UpFilePath=" + uppath + "&IsWater=" + iswater;
    }
    //判断是否生成宿略图
    if (arguments.length == 5) {
        sendUrl = "../../FileUp/UpLoadFile?action=" + action + "&ReFilePath=" + repath + "&UpFilePath=" + uppath + "&IsWater=" + iswater + "&IsThumbnail=" + isthumbnail;
    }
    //自定义上传路径
    if (arguments.length == 6) {
        sendUrl = filepath + "FileUp/UpLoadFile?action=" + action + "&ReFilePath=" + repath + "&UpFilePath=" + uppath + "&IsWater=" + iswater + "&IsThumbnail=" + isthumbnail;
    }
    //开始提交
    $("#form1").ajaxSubmit({
...
function AttachUpload(repath, uppath) {
    var submitUrl = "../../FileUp/UpLoadFile?action=AttachFile&UpFilePath=" + uppath;
    //开始提交
    $("#form1").ajaxSubmit({
        beforeSubmit: function (formData, jqForm, options) {
            //隐藏上传按钮
...
function AttachUpdate(repath, uppath) {
    ...
    var submitUrl = "../../FileUp/UpLoadFile?action=AttachFile&UpFilePath=" + btnNewName;
    //开始提交
    $("#form1").ajaxSubmit({
        beforeSubmit: function (formData, jqForm, options) {
            //隐藏上传按钮
