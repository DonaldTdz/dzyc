...
*	初始化SWFUpload上传控件
* ****************************************** */
function InitSWFUpload(upurl, uppath, upsize, flashurl, iswater, isthumbnail, file_types) {
    ...
    var swfu = new SWFUpload({
        ...
        file_types: file_types, //"*.jpg;*.jpge;*.png;*.gif;*.xls",
        file_types_description: "上传文件格式",
        file_upload_limit: "0",
...
function uploadSuccess(file, serverData) {
    ...
    try {
        ...
        if (jsonstr.msg == '1') {
           // $('#FileIds').val(jsonstr.FileId);
          //  alert($('#FileIds').val());
            FileIds = jsonstr.FileId;
...
function addImage(bigSrc, smallSrc) {
    ...
    var newImg = $('<div onclick="focus_img(this);" class=\"img_box ' + claname + '">' +
        ...
        '<span class="remark"><i>已上传...</i></span></div>').appendTo(newLi);
